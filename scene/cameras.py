...
class Camera(nn.Module):
    def __init__(self, resolution, colmap_id, R, T, FoVx, FoVy, depth_params, image, invdepthmap,
                 ...
                 train_test_exp = False, is_test_dataset = False, is_test_view = False, specular_mask=None,
                 intrinsic=None
                 ):
        ...
        self.projection_matrix = getProjectionMatrix2(znear=self.znear, zfar=self.zfar, cx=cx, cy=cy, fx=fx, fy=fy, W=self.image_width, H=self.image_height).transpose(0,1).cuda()
        # print('projection_matrix', self.projection_matrix)
        self.full_proj_transform = (self.world_view_transform.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))).squeeze(0)
        self.camera_center = self.world_view_transform.inverse()[3, :3]
        # print('camera_center', self.camera_center)
    def modify_camera(self, R, T):
        """
        to rotate the camera, use the following:
        """
        R = R
        T = T
...
class EndoCamera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, depth, mask, gt_alpha_mask,
        ...
        super(EndoCamera, self).__init__()
